...
5. **Every batch operation is idempotent and keyed**, with a `job_runs` record and a resumable
   cursor.
...
9. **Module boundaries follow the layer order** in `docs/03-architecture.md` §3 and are
   linter-enforced.
...
And answer one question in the PR: **how does a row in this table get erased or anonymised?** See
the retention table in `docs/17-legal-and-compliance.md` §4. No answer, no merge.
...
`17` legal, privacy, retention, operator access · `18` incident response and runbooks · `19`
company, tax and invoicing.
