 ...
 *
 * Same rules as the link-based verification template: `BRAND` only, text + HTML,
 * no remote assets, escape attacker-controlled names.
 */
export function signupOtpTemplate(input: {
  ...
  const text = [
    ...
    '',
    'If you did not start a signup, you can ignore this message.',
    '',
    `— ${BRAND.name}`,
  ].join('\n');
      ...
      <tr>
        <td style="padding:32px;">
          ...
          <p style="margin:0 0 16px;font-size:14px;line-height:1.6;">
            Your verification code for ${escapeHtml(BRAND.name)} is:
          </p>
