...
describe('schoolSlugFromHost', () => {
  it('reads the slug from a subdomain', () => {
    expect(schoolSlugFromHost('beacon.ilm.pk', DOMAIN)).toBe('beacon');
    expect(schoolSlugFromHost('beacon.ilm.pk:443', DOMAIN)).toBe('beacon');
    expect(schoolSlugFromHost('BEACON.ILM.PK', DOMAIN)).toBe('beacon');
  });

  it('returns nothing for the apex domain', () => {
    expect(schoolSlugFromHost('ilm.pk', DOMAIN)).toBeUndefined();
  });
  ...
  it('returns nothing for a reserved subdomain', () => {
    ...
    for (const reserved of ['www', 'api', 'admin', 'status']) {
      expect(schoolSlugFromHost(`${reserved}.ilm.pk`, DOMAIN)).toBeUndefined();
    }
  });

  it('refuses a nested label rather than taking the first one', () => {
    // Treating `a.b.ilm.pk` as school `a` would let anyone holding a wildcard
    // certificate select any school by prefixing labels.
    expect(schoolSlugFromHost('a.b.ilm.pk', DOMAIN)).toBeUndefined();
  });
  ...
  it('refuses a host outside the configured domain', () => {
    ...
    expect(schoolSlugFromHost('beacon.notilm.pk', DOMAIN)).toBeUndefined();
    expect(schoolSlugFromHost('ilm.pk.evil.test', DOMAIN)).toBeUndefined();
  });
