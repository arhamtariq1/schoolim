...
**R1** and **R2** are existential and technical — the architecture already addresses them, so the
job is to keep the tests green. **R4** and **R5** are existential and behavioural — no architecture
...
### Taken during the Phase 0 build (2026-08-25), for review

Decisions the implementation forced. Each is reversible; none is silent.

| Decision                                                                                          | Why it was forced                                                                                                                                                                                                                                                                                                                                     | Reversible by                                             |
| ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| `apps/web` renamed **`apps/portal`**                                                              | Nine docs said `web`, one said `portal`, and "web" is a poor name against `admin` — both are web                                                                                                                                                                                                                                                      | A rename                                                  |
| Shared packages build to **CommonJS `dist`** rather than exporting ESM TypeScript source          | NestJS needs CJS (legacy decorators + `emitDecoratorMetadata`) and could not consume ESM source                                                                                                                                                                                                                                                       | Making the API ESM, once NestJS is comfortable there      |
| Prisma generator set to **`moduleFormat = "cjs"`**                                                | Prisma 7 generates ESM whose client uses `import.meta.url`, which forces Node to parse it as ESM                                                                                                                                                                                                                                                      | Same as above                                             |
| `nestjs` tsconfig moved to **`moduleResolution: Node16`**                                         | `Node10` ignores `exports` maps and mis-resolved `@fastify/*` types                                                                                                                                                                                                                                                                                   | —                                                         |
...
| **The API is served on each app's own hostname** under `/api/v1`, proxied by a Next route handler | The browser must reach the API first-party or the session cookie needs `SameSite=None` and CORS with credentials. Serving it on the school's host also means the `Host` header names the school, so tenant resolution for browser traffic is the design in `09` §2 with nothing added. In production the ingress does this and the handler never runs | Pointing the browser at `api.<domain>` and accepting CORS |
| **`COOKIE_DOMAIN` unset by default**, so session cookies are host-only                            | `Domain=<apex>` shares one cookie with every tenant subdomain, so a browser signed into one school transmits its token to every other school's hostname. Host-only means it is never sent there at all                                                                                                                                                | Setting the variable, if a deployment ever needs it       |
| **Supabase reached through the IPv4 pooler**, never `db.<ref>.supabase.co`                        | The direct host publishes only an AAAA record; Supabase moved direct connections to IPv6-only and charges for IPv4. On a network without IPv6 it fails as `DatabaseNotReachable`, which reads like an outage                                                                                                                                          | The IPv4 add-on, or Railway                               |
| **One `.env` at the workspace root**, loaded by walking up to `pnpm-workspace.yaml`               | `import 'dotenv/config'` reads from the _current_ package, which had grown three copies of the same credentials. When one changed, the API dialled the old database while the CLI used the new one                                                                                                                                                    | —                                                         |
| **Platform sessions get their own table**, not a nullable `school_id` on `sessions`               | `sessions.school_id` is `NOT NULL` and carries the `tenant_isolation` policy. Relaxing it so a handful of staff accounts could share the table would put a permanent hole in what isolates every school                                                                                                                                               | —                                                         |

**One divergence left open, deliberately.** `docs/12` §1 mandates `exactOptionalPropertyTypes: true`
and `verbatimModuleSyntax`; `packages/config/typescript/base.json` has the former `false` and omits
the latter. Turning them on is cheap now and a slog once there are features. **Needs a decision:**
align the config to the doc, or relax the doc to match the config.
...
|        | Decision                | Status                                                                                                                                          |
| ------ | ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **D1** | API hosting             | **Open — blocks only the P0 _deploy_, not the build.** Decide by the end of Phase 0. Database is settled: Supabase now → Railway at P5.         |
...
### D2 — Pricing model _(shapes the super-admin schema, needed by Phase 5)_

- **(a)** Flat tiers with a student cap, billed annually — recommended; avoids monthly headcount
  arguments
- **(b)** Per-student per-month — scales with value, invites disputes
- **(c)** Per-term billing — matches how schools think about money here

The schema in `07` §1 supports all three; you only need to choose which to _ship_.

### D3 — First customer strategy _(shapes Phase 1–2 priorities)_

- **(a)** Migrate your existing school onto the new product — a real reference and real feedback,
  but they will pull you toward their old requirements. Rule R1 becomes hard to hold.
...
### D4 — Product name and domain _(needed before Phase 1; cheap now, expensive later)_
...
- **(b)** Bilingual from day one — a real differentiator for smaller schools, roughly 15% extra
  effort across every screen

### D6 — Family voucher grouping

Should siblings receive one combined voucher or one per student? Schools genuinely differ.
**Recommendation: build per-student in v1 and make combined a Phase 6 setting** — combined vouchers
complicate allocation, part payments and reconciliation significantly, and doing it badly early
would violate several invariants.
